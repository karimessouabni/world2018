...
export class CompetitionProvider {
  ...
  getCompetitions2017() {
    ...
    return new Promise(resolve => {
      this.http.get('http://169.254.77.227:8080/api/allCompetitions')
        .map(res => res.json())
  ...
  getFixturesByDay(day: any) {
    return new Promise(resolve => {
      this.http.get('http://169.254.77.227:8080/api/fixtures/'+day)
        .map(res => res.json())
        .subscribe(matches => {
          this.matches = matches;
          resolve(this.matches);
  ...
  */
  getFixturesByDayAndCompet(day: any, compet: any) : any {
    return new Promise(resolve => {
      this.http.get('http://169.254.77.227:8080/api/fixtures/'+day+'/'+compet)
        .map(res => res.json())
        .subscribe(data => {
          this.data = data;
          resolve(this.data);
  ...
  getTeamsById(idTeam: any) {
    return new Promise(resolve => {
      this.http.get('http://169.254.77.227:8080/api/team/'+idTeam)
        .map(res => res.json())
        .subscribe(data => {
          this.data = data;
          resolve(this.data);
